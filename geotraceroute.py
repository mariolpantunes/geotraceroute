...
def main(url):
    if 'container' in os.environ:
        ...
        command = f'traceroute -n -F -I -w 1 {url}'
    ...
    ips = [[ip for ip in line.split(' ') if valid_ip(ip)] for line in result.split('\n')]
    ips = [ip[0] for ip in ips if len(ip) > 0]

    ips[0] = get_public_ip()
    logger.info(f'IPs {ips}')

    geotrace = [resolve_ip(ip) for ip in ips]
    ...
    for point in valid_points:
        ip, lat, lon = point
        ax.text(lon + 0.5, lat + 0.5, ip,  # Offset text slightly
